...
- 왜 가능한가
  ...
  - $X\in\mathbb{R}^{n\times p}$, 열 평균 벡터 $\bar{x}\in\mathbb{R}^p$, $v\in\mathbb{R}^p$일 때
    $$(X-\mathbf{1}\bar{x}^\top)v = Xv - \mathbf{1}(\bar{x}^\top v), \qquad \mathbf{1}\in\mathbb{R}^n$$
  - 중심화한 행렬을 직접 만들지 않고 곱을 계산 ⟹ 원본 $X$의 희소성 유지
